...
import utils
from train_helpers import set_up_hyperparams, load_vaes, load_opt, accumulate_stats, save_model
import pickle
from vae import VAE
from train_helpers import restore_params

def iwae_calc(x, H, vae, logprint, K = 200):
    '''
    ...
    for i in range(K):
        stats = vae.forward(x[None, :], x[None, :])
...
def elbo_calc(x, H, vae, logprint, K = 200):
    '''
        Pass image through the VAE and calculate the log likelihood 
...
def iwae_calc_manual(x, H, vae, logprint, K = 200, idx=0):
    x = x[0].cuda()
    sum = 0
    iwae_list = []
    for i in range(K):
        ...
        try:
            ...
            xhat, log_p_xGz,_ = vae.forward_samples_set_latents(mb, zs, t=1.0, prob_vector=True, x = x)
...
def load_trained_vaes(H, logprint, epoch):
    ...
    if model_type != 'large' and model_type != 'larger' and model_type != 'medium' and model_type != 'small':
...
def main_train():
    ...
    epochs_g1 = [250, 225, 200, 175]
    epochs_g2 = [150, 125, 100]
    epochs_g3 = [75, 50, 25, 0]
    if H.group == 'g1':
        epochs = epochs_g1
    elif H.group == 'g2':
        epochs = epochs_g2
